                ...
                switch (msg.MessageType) {
                    case NetIncomingMessageType.DiscoveryRequest:
                        ...
                        if (Accounts_DB.BannedIP(msg.SenderEndPoint.Address.ToString()) == true) {
                            FileLog.WriteLog("Warning: Attempted IP Banned " + msg.SenderEndPoint.Address);
                    ...
                    case NetIncomingMessageType.StatusChanged:
                        ...
                        if (status == NetConnectionStatus.Disconnected) {
                            ...
                            if (Settings.LogSystem == 1) { FileLog.WriteLog($"Status changed to disconnected: {pData?.ID} {pData?.Account} {msg.SenderEndPoint.Address} {pData.HexID}"); }
                            if (!Settings.LogSystemScreen) { FileLog.WriteLog($"Status changed to disconnected: {pData?.ID} {pData?.Account} {msg.SenderEndPoint.Address} {pData.HexID}", Color.Coral); }
